...
本项目当前属于**可投入个人和小团队生产使用的工程化桌面视频标注工具**：已经具备工程 v2、跨机器迁移、自动备份、版本快照、质量检查、批量标注、审核状态、统计报表、导出队列和插件注册基础。

接下来集中完成五项产品化升级：插件导出闭环、审核统计与质量门禁、PyInstaller 打包验证、大工程性能测试、审核历史与协作模型。详细执行顺序见 docs/PRODUCTIZATION_PLAN.md，系统流程见 docs/PROJECT_FLOW.md。

## 项目流程图

~~~mermaid
flowchart TD
    A[启动应用] --> B[加载偏好设置与内置导出器]
    B --> C{加载或新建工程}
    C --> D[导入视频并读取元数据]
    D --> E[创建时间片段与标签]
    E --> F[保存工程、备份和快照]
...
1. **插件导出闭环**：选择导出器、片段范围和目标目录，展示进度与错误。
2. **审核统计与质量门禁**：增加审核完成率、通过率，并在导出前拦截质量问题。
3. **PyInstaller 打包验证**：验证 Qt 依赖、资源、FFmpeg 路径和启动脚本。
4. **大工程性能测试**：验证大量视频、片段、筛选、保存、加载和统计。
5. **审核历史与协作模型**：记录审核人、时间、意见和状态变更。
